...
- 执行 `tailscale ip -4` 获取本机 Tailscale IP，并用它启动触发 API；
- 把 PID 写入 `tt-trigger.pid`，把日志写入 `logs` 目录。

复制窗口显示的 Token，并记录启动窗口输出的 `Tailscale API` 地址。以后可以从 `config.json` 查看 Token。
...
```text
http://TAILSCALE_IP:8788/trigger?token=YOUR_TOKEN&symbol=BTC
```
...
```text
http://TAILSCALE_IP:8788/trigger?token=YOUR_TOKEN&symbol=BTC&addPair=true
```

包含特殊字符的 symbol 示例：

```text
http://TAILSCALE_IP:8788/trigger?token=YOUR_TOKEN&symbol=BG-P%3ASIREN%2FUSDT%2BOD-S%3ASIREN%2FUSDT&addPair=true
```

可以通过以下命令查看实际 Tailscale IP：
...
```bash
curl -X POST 'http://TAILSCALE_IP:8788/webhook' \
...
通过 Tailscale IP 的 8788 端口访问时只返回 API 存活状态：
